...
def unet(img_w,img_h,n_label=5, data_format='channels_first'):
    inputs = keras.layers.Input(shape=(3,img_w,img_h))
    x = inputs
    depth = 4
    features = 64
    skips = []
...
########################################################################################################
#Attention R2U-Net
def att_r2_unet(IMAGE_WIDTH, IMAGE_HEIGHT, n_label, data_format="channels_last"):
    IMAGE_HEIGHT=512
    IMAGE_WIDTH=512
    in_channels=3
    ...
    x = rec_res_block(x, features, data_format=2)
